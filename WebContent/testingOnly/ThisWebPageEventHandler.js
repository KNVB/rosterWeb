class ThisWebPageEventHandler
{
	...
	_handleArrowKeyEvent(event,yOffset,xOffset)
	{
		...
		if (!this.selectedRegion.isClear())
		{
			...
			var index;
			var maxRowCount=this.rosterSchedulerTable.itoList.length*2;
			var orgIndex=$.inArray(theCell,this.cursorCells);
			var nextCell,xSpan;
			
			var newX=orgIndex % this.rosterSchedulerTable.dateObjList.length;
			var newY=(orgIndex-newX)/this.rosterSchedulerTable.dateObjList.length;
			
			newX+=xOffset;
			if (newX>=this.rosterSchedulerTable.dateObjList.length)
				newX=0;
			else
				if (newX<0)
					newX=this.rosterSchedulerTable.dateObjList.length-1;
			newY+=yOffset;
			if (newY>=maxRowCount)
				newY=0;
			else
			...
			index=newX+newY*this.rosterSchedulerTable.dateObjList.length;
