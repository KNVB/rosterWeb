...
export default function EditableShiftCell(props){
    ...
    let updateValue=(e)=>{
        ...
        e.preventDefault();
        let oldValue=contextValue.rosterList[props.itoId].presentValue.shiftList[e.target.cellIndex-contextValue.systemParam.noOfPrevDate];
        /****************************************************************/
        ...
        if (e.target.textContent!==oldValue){
            let temp=JSON.parse(JSON.stringify(contextValue.rosterList[props.itoId].presentValue));
            temp.shiftList[e.target.cellIndex-contextValue.systemParam.noOfPrevDate]=e.target.textContent;
            contextValue.rosterList[props.itoId].set(temp);
