export default function useShift() {
    const getITOStat = (activeShiftInfoList, noOfWorkingDay, itoRoster) => {
      ...
      Object.keys(itoRoster.shiftList.presentValue).forEach(date => {
        let item = itoRoster.shiftList.presentValue[date];
    ...
    const getShiftCountList = itoRoster => {
      let aShiftCount = 0,
        ...
        cShiftCount = 0,
        dxShiftCount = 0;
      Object.keys(itoRoster.shiftList.presentValue).forEach(key => {
        let item = itoRoster.shiftList.presentValue[key];
        let shiftTypeList = item.split("+");
        shiftTypeList.forEach(shiftType => {
          if (itoRoster.availableShiftList.includes(shiftType)) {
            switch (shiftType) {
