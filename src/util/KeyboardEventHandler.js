export default function KeyboardEventHandler(itemList, updateItemList) {
        ...
        if (itemList.rosterSchedulerTableUtil.isFirstInput()) {
            switch (e.key) {
                ...
                case "y"://handle redo 
                    if (e.ctrlKey) {
                        e.preventDefault();
                        itemList.rosterSchedulerUtil.reDo();
                ...
                case "z"://handle undo 
                    if (e.ctrlKey) {
                        e.preventDefault();                        
                        itemList.rosterSchedulerUtil.unDo();
                        updateItemList({ type: "refresh" });
                    }
                    break;
                default:
    ...
    const handleDelKeyEvent = (e) => {
        ...
        itemList.rosterSchedulerUtil.deleteSelectedData(selectedLocation, itemList.noOfWorkingDay, itemList.calendarDateList.length);
        updateItemList({ type: "refresh" });
    }
    const handleEscKeyEvent = (e) => {
        e.preventDefault();
        itemList.rosterSchedulerTableUtil.clearCopiedRegion();
        itemList.rosterSchedulerUtil.clearCopiedData();
        updateItemList({ type: "refresh" });
    }
